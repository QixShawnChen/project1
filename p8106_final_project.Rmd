...
```{r}
set.seed(2022)
# Randomly sample 3500 data points without replacement from the dataset.
...
training_data = df_sample[training_tag, ]
training_predictors_x = as.data.frame(model.matrix(cardio ~ ., training_data)[, -1]) %>%
  janitor::clean_names()
training_outcome_y = training_data$cardio


# For test dataset
test_data = df_sample[-training_tag, ]
test_predictors_x = as.data.frame(model.matrix(cardio ~ ., test_data)[, -1]) %>%
  janitor::clean_names()
...
test_pred = rep("diseased", length(test_pred_prob))

test_pred[test_pred_prob > 0.5] = "nondiseased"
...
mars_test_pred_prob_df = predict(mars_model, newdata = test_data,
                         type = "prob"
                         )

mars_test_pred_prob = mars_test_pred_prob_df$nondiseased

mars_test_pred = rep("diseased", length(mars_test_pred_prob))

mars_test_pred[mars_test_pred_prob > 0.5] = "nondiseased"
...
#ConfusionMatrix
test_pred_rf = rep("diseased", length(rf_pred ))

test_pred_rf[rf_pred > 0.5] = "nondiseased"
...
test_pred_svm = rep("diseased", length(svm_pred))

test_pred_svm[svm_pred > 0.5] = "nondiseased"
